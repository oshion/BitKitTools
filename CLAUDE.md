...
17. **기존 기능 수정/신규 추가 시 경로 판단** — 툴/화면을 고치거나 새로 추가해달라는 요청을 받으면 범위에 따라 둘 중 하나를 택한다:
    ...
    - **스펙이 실질적으로 바뀌는 확장, 또는 신규 툴/화면 추가처럼 규모가 큰 `docs/screens/` 작성·수정이 필요한 작업**(새 입력/출력, 새 disclaimer 판단, 별도 AC로 검증할 만한 작업 단위) → `docs/screens/{화면명}.md`를 작성/갱신하며 **사용자와 기획 내용을 논의하고 확정**한다. **Claude는 여기서 멈춘다 — 이 범위의 작업에서는 `/harness` step 설계, `phases/` 파일 생성, `execute.py` 실행을 스스로 진행하지 않는다.** screens 문서 확정 후 `/harness` 실행 여부와 시점은 사용자가 직접 판단한다.
